...
module.exports.copyTemplateFile = async (opts) => {
  ...
  let fileRelativePath = path.relative(source, file)
  // special case to deal with package.json
  if (fileRelativePath === 'package-json') {
    fileRelativePath = 'package.json'
  } else if (fileRelativePath === 'gitignore') {
...
module.exports.initGitRepo = async (opts) => {
  ...
  const cmd = `git init && git add . && git commit -m "created by ${pkg.name}@${pkg.version}"`
